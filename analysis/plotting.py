...
def create_scatter(
    ...
    args_dict = {"x": x[:, 0], "y": y[:, 0], "z": z[:, 0], "mode": mode, "marker": {}}
    if marker_size:
...
def plot_traces(fig_traces, height=500, width=600, title=None):
    """Constructs"""
    layout_args = {
        "height": height,
        "width": width,
...
def plot_traces_and_layout(fig_traces, layout):
    fig = go.Figure(data=fig_traces, layout=layout)
    fig.show()
...
def create_dynamic_layout(height=500, width=600, title=None, scene_range=None):
    layout_args = {
        "height": height,
        "width": width,
        "autosize": False,
                ...
                "buttons": [
                    {
                        "args": [
                            None,
                            {"frame": {"duration": 30}, "transition": {"duration": 30}},
                        ],
                        "label": "Play",
                        "method": "animate",
                    },
...
def plot_sample_grid(samples, num_res, motif_bb_3d=None, true_bb_3d=None):
    ncols, nrows = 3, 3
    fig = make_subplots(
        rows=nrows, cols=ncols, specs=[[{"type": "surface"}] * nrows] * ncols
    )
    fig.update_layout(
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            sample_bb_3d = create_scatter(
                ...
                marker_size=3,
                opacity=1.0,
                name=f"Sample {i*nrows+j}: length_{num_res[b_idx]}",
            )
            fig.add_trace(sample_bb_3d, row=i + 1, col=j + 1)

            if motif_bb_3d is not None:
                fig.add_trace(motif_bb_3d, row=i + 1, col=j + 1)
            if true_bb_3d is not None:
                fig.add_trace(true_bb_3d, row=i + 1, col=j + 1)
...
def plt_3d(x, ax, color=None, s=20, mode="scatter", ax_lim=None):
    ...
    if mode == "scatter":
        ax.scatter3D(x[:, 0], x[:, 1], x[:, 2], color=color, s=s)
    elif mode == "line":
        ax.plot3D(x[:, 0], x[:, 1], x[:, 2], alpha=0.2)
    else:
