...
def igso3_expansion(omega, eps, L=1000, use_torch=False):
    ...
    # Todo: Investigate this further, the vectorised version should be equal and faster but we get some warnings about
    #  overflow and division by zero when we use this function.
    # l = np.arange(L)
    # p = np.sum(
    #     (2 * l + 1)
    #     * np.exp(-l * (l + 1) * eps ** 2 / 2)
    #     * np.sin(omega[:, None] * (l + 1 / 2))
    #     / np.sin(omega[:, None] / 2),
    #     axis=1
    # )
