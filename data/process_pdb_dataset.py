...
parser.add_argument(
    "--max_resolution", help="Max resolution of files.", type=float, default=5.0
)
parser.add_argument("--max_len", help="Max length of protein.", type=int, default=512)
parser.add_argument(
    "--num_processes", help="Number of processes.", type=int, default=100
)
parser.add_argument(
    "--write_dir",
...
def _retrieve_mmcif_files(
    ...
    for subdir in tqdm(os.listdir(mmcif_dir)):
        ...
        if debug and total_num_files >= 100:
            ...
            break
    print(f"Processing {len(all_mmcif_paths)} files our of {total_num_files}")
...
def process_mmcif(mmcif_path: str, max_resolution: int, max_len: int, write_dir: str):
    ...
    for chain_id, chain in struct_chains.items():
        ...
        chain_dict = du.center_and_scale_coords(chain_dict)
...
def process_serially(all_mmcif_paths, max_resolution, max_len, write_dir):
    all_metadata = []
    for i, mmcif_path in enumerate(all_mmcif_paths):
        try:
            start_time = time.time()
            metadata = process_mmcif(mmcif_path, max_resolution, max_len, write_dir)
...
def process_fn(
    mmcif_path, verbose=None, max_resolution=None, max_len=None, write_dir=None
):
    try:
        start_time = time.time()
        metadata = process_mmcif(mmcif_path, max_resolution, max_len, write_dir)
...
def main(args):
    ...
    if args.num_processes == 1 or args.debug:
        all_metadata = process_serially(
            all_mmcif_paths, args.max_resolution, args.max_len, write_dir
        )
    else:
