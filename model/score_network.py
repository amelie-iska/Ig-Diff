...
class Embedder(nn.Module):
    def __init__(self, model_conf):
        ...
        self._use_chain_embedding = model_conf.use_chain_embedding
        ...
        if model_conf.use_chain_embedding:
            self.chain_embedder = nn.Linear(1, node_embed_size)
        self.node_embedder = nn.ModuleList(
            [
    ...
    def forward(
        ...
        *,
        ...
        Args:
            ...
            chain_idx: [..., N] Chain index for each residue.
            t: Sampled t in [0, 1].
        ...
        node_embed = torch.cat(node_feats, dim=-1).float()
        for i, node_embed_layer in enumerate(self.node_embedder):
            node_embed = node_embed_layer(node_embed)
            if i == 0 and self._use_chain_embedding:
                node_embed += self.chain_embedder(chain_idx.unsqueeze(dim=-1).float())
